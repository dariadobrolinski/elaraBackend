...
def classifyCondition(symptoms: list[str] = ['Cancer']):
  client = genai.Client(
      vertexai=True,
...
INSTRUCTION:
• The user will send you a JSON object with a single key, `\"inputs\"`, whose value is an array of symptom strings. 
• Your job is to return **only** a JSON object with a single key, `\"outputs\"`, whose value is a **dictionary** mapping **each symptom** to the **one best** matching medical-use classification from the dictionary above. 
...
 }
  ...
  contents = [
    types.Content(
      role="user",
      parts=[
        types.Part.from_text(text=f"""{symptoms}""")
      ]
    ),
  ]
